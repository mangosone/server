...
**MangosOne** is the **The Burning Crusade World of Warcraft** server of the MaNGOS project: an open-source
C++ game server that emulates retail patches **2.4.3**. It **does not** support
2.5.x (Classic) or any later expansion. Game and account data live in **MySQL / MariaDB**.
...
- **`mangosone/database`** — world/realm/character SQL plus incremental migrations. Schema or
...
```sh
git clone --recursive https://github.com/mangosone/server.git
cd server
...
- **Database changes live in `mangosone/database`**, applied as transactional, idempotent
...
- Compatibility target is **2.4.3 only** — do not introduce later-expansion assumptions.
