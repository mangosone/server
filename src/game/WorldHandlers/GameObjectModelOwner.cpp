...
bool GameObjectModel::Initialize(const GameObject* pGo, uint32 displayId)
{
    ...
    m_owner = pGo;
    // TBC has no phasing. The field stays, because the collision index is shared
    // with the cores that do phase, and every body sits in the only phase there is
    // so the filter always passes.
