...
void PlayerMenu::SendQuestGiverQuestDetails(Quest const* pQuest, ObjectGuid guid, bool ActivateAccept) const
{
    ...
    data << Details;                                        // Quest details
    data << Objectives;                                     // Quest objectives
    data << uint32(ActivateAccept ? 1 : 0);                 // Auto finish flag
    ...
    for (uint32 i = 0; i < QUEST_EMOTE_COUNT; ++i)
    {
        ...
        data << uint32(pQuest->DetailsEmoteDelay[i]);       // Emote delay in ms
    }
...
void PlayerMenu::SendQuestQueryResponse(Quest const* pQuest) const
{
    ...
    data << uint32(pQuest->GetSuggestedPlayers());          // suggested players count
    data << uint32(pQuest->GetRepObjectiveFaction());       // reputation objective faction
    data << uint32(pQuest->GetRepObjectiveValue());         // reputation objective value
    ...
    data << uint32(pQuest->GetRewMoneyMaxLevel());          // reward money at max level
    data << uint32(pQuest->GetRewSpell());                  // reward spell
    data << uint32(pQuest->GetRewSpellCast());              // casted spell
    data << uint32(MaNGOS::Honor::hk_honor_at_level(GetMenuSession()->GetPlayer()->getLevel(), pQuest->GetRewHonorableKills())); // rewarded honor points
    data << uint32(pQuest->GetSrcItemId());                 // source item id
    data << uint32(pQuest->GetQuestFlags());                // quest flags
    data << uint32(pQuest->GetCharTitleId());               // character title id
...
void PlayerMenu::SendQuestGiverOfferReward(Quest const* pQuest, ObjectGuid npcGUID, bool EnableNext) const
{
    ...
    // Add rewarded honor points to the packet. Multiply with 10 to satisfy client
    data << uint32(10 * MaNGOS::Honor::hk_honor_at_level(GetMenuSession()->GetPlayer()->getLevel(), pQuest->GetRewHonorableKills()));
...
void PlayerMenu::SendQuestGiverRequestItems(Quest const* pQuest, ObjectGuid npcGUID, bool Completable, bool CloseOnCancel) const
{
    ...
    // If there are no request items text, send the quest offer reward instead
