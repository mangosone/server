...
// The searcher and worker Visit() bodies used to live here: one hand-written function
// per (result type x grid type x search mode), around 40 of them, identical apart from
// the names. They are now a handful of templates in GridNotifiers.h, which pick the grid
// types they care about with `if constexpr (visits_v<Result, T>)` — see the comment there.
