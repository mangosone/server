...
{
    sLog.outString("Initializing AI Playerbot by ike3, based on the original Playerbot by blueboy and updated by DarthMesut");
    ...
    randomGearLoweringChance = config.GetFloatDefault("AiPlayerbot.RandomGearLoweringChance", 0.25f);
    randomBotMaxLevelChance = config.GetFloatDefault("AiPlayerbot.RandomBotMaxLevelChance", 0.25f);
...
void PlayerbotAIConfig::CreateRandomBots()
{
    ...
    LoginDatabase.PExecute("UPDATE account SET expansion = '%u', playerbot = %u where username like '%s%%'", 1,true, randomBotAccountPrefix.c_str());
