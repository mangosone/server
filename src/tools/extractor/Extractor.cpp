...
int main(int argc, char** argv)
{
    ...
    g_console.Start(opt.src, opt.dest, "2.4.3");
    ...
    StormLibArchive mpq;
    const int opened = mpq.OpenClientData(opt.src, ClientArchives243(),
                                          ClientLocaleArchives243(), opt.locale);
    if (!opened)
    {
    ...
    if (opt.dbc && opt.allLocales)
    {
        for (const std::string& loc : FindLocales(opt.src))
        {
            ...
            if (!other.OpenClientData(opt.src, ClientArchives243(),
                                       ClientLocaleArchives243(), loc))
            {
